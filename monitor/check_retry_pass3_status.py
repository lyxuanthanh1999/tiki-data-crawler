#!/usr/bin/env python3
"""
Dashboard giám sát thời gian thực tiến độ Phase 3 (Quét vét lỗi 83,804 ID).
Theo dõi chi tiết 5 tiến trình, 84 batch và số lượng sản phẩm 'vớt' được.
"""
...
PARTS_DIR = Path("data/output/retry_pass3/parts")
TOTAL_IDS = 83804
TOTAL_BATCHES = 84
BATCH_RANGES = [
    (1, 1, 17),
...
def get_running_processes():
    ...
    for line in res.stdout.strip().split("\n")[1:]:
        ...
        if len(parts) >= 4 and "main.py" in parts[3] and "retry_pass3" in parts[3]:
            pid = parts[0]
            cpu = parts[1]
            etime = parts[2]
            args = parts[3]
            for p_num, start_b, end_b in BATCH_RANGES:
                if f"--start-batch {start_b} " in f"{args} ":
                    processes[p_num] = {
                        "pid": pid,
                        "cpu": cpu,
                        "etime": etime,
...
def main():
    ...
    print("=" * 89)
    print("📊 BẢNG TRẠNG THÁI PHASE 3: QUÉT LỖI 83,804 ID (REAL-TIME DASHBOARD)")
    ...
    for p_num, start_b, end_b in BATCH_RANGES:
        ...
        if all_done:
            ...
            done_in_proc = max(0, curr_b - start_b)
            status_str = f"⏹️ Tạm dừng ở Batch #{curr_b:04d} ({done_in_proc}/{total_batches_in_proc} batches xong)"
            pid_str = "  N/A"
            cpu_str = " 0.0%"
            etime_str = "     --"

        print(f"Process {p_num} ({range_str}) | PID: {pid_str} | CPU: {cpu_str} | Chạy: {etime_str} | {status_str}")

    print("-" * 89)
    print(f"📦 Tổng tiến độ Phase 3: {done:,} / {TOTAL_IDS:,} ID ({pct:.2f}%)")
    print(f"   🌟 Sản phẩm sạch 'vớt' được : {saved:,} ({(saved/TOTAL_IDS*100) if TOTAL_IDS else 0:.2f}%)")
    print(f"   ❌ Xác nhận là True 404     : {failed:,} ({(failed/TOTAL_IDS*100) if TOTAL_IDS else 0:.2f}%)")
    print(f"   ⏳ Số ID còn lại cần quét   : {remaining:,} ({(remaining/TOTAL_IDS*100) if TOTAL_IDS else 0:.2f}%)")
    print("=" * 89)
